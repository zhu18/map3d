...
*/
import { Color, Object3D, Texture, SpriteMaterial, Sprite, MeshBasicMaterial, PlaneGeometry, Mesh, EventDispatcher, Vector3, MOUSE, Quaternion, Spherical, Vector2, Raycaster, ShaderLib, ShaderMaterial, FrontSide, AdditiveBlending, Geometry, LineBasicMaterial, Group, Line as Line$1, Shape, ExtrudeGeometry, MeshPhongMaterial, MeshLambertMaterial, BoxGeometry, QuadraticBezierCurve3, BufferGeometry, BufferAttribute, Points, TextureLoader, MeshStandardMaterial, NormalBlending, DoubleSide, CylinderGeometry, Scene, PerspectiveCamera, WebGLRenderer, Fog, AmbientLight, DirectionalLight, SpotLight, DirectionalLightHelper, SpotLightHelper, GridHelper, AxisHelper } from 'three';
 ...
 * 3D文字
 */
class Font3D$1 extends Object3D {
  constructor(txt, opt) {
    super();
    this.type = "Font3D";
    this.name = txt;
  ...
  getText2D(text, opt) {
    ...
    if (opt.follow) {
      ...
      this._textObj = new Sprite(spriteMaterial);
    } else {
      ...
      this._textObj = new Mesh(geometry, material);
    }
 ...
 */
ShaderLib.createShaderMaterial = function (shader, side, blending) {
...
class Area extends Object3D {
  ...
  getText(pros) {
    if (!pros.cp) return;
    let text = new Font3D$1(pros.name, pros.textStyle);
  ...
  getLine(coords, pros) {
    ...
    coords.forEach(coord => {
      ...
      for (let i = 0, l = pts.length; i < l; i++) {
        line.vertices.push(new Vector3(pts[i].x, pts[i].y, this.userData.extrude.amount + this.userData.extrude.amount / 100));
      }
      let lineMesh = new Line$1(line, material);
      lines.add(lineMesh);
    });
    return lines;
  }
  ...
  getExtrudeGeometry(pts) {
    ...
    let geo = new ExtrudeGeometry(shape, extrude);
    return geo;
  }
  /**
   * 拉伸块高度
  ...
  getGeoMesh(geo, pros) {
    ...
    mateOption.opacity = typeof pros.opacity === 'undefined' ? this.userData.opacity : pros.opacity; //mateOption.wireframe =true
    geo.computeFlatVertexNormals();
    let geoMesh = null; //   var shader = {
    //     uniforms:
    //     {
 ...
 * @extends THREE.Mesh
 */
class DataRange extends Object3D {
  constructor(pros) {
    super(pros);
    this.type = "DataRange";
    this.name = pros.name;
    Object.assign(this.userData, pros);
    let boxGeo = new BoxGeometry(pros.width, pros.height, pros.extrudeHeight);
    let boxMate = new MeshPhongMaterial({
      color: pros.color
    });
    let range = new Mesh(boxGeo, boxMate);
    this.position.y = this.position.y - DataRange.count * (pros.height + pros.spacing);
    if (pros.showName) {
      let txt = Font3D.create(pros.name, {
        color: '#ffffff'
      });
...
class Mark extends Sprite {
  ...
  constructor(pros) {
    ...
    this.scale.set(size, size, 1); //console.log(size);
...
class Line extends Line$1 {
  ...
  constructor(pros) {
    ...
    let v3s = [new Vector3(x1, y1, pros.extrudeHeight), new Vector3((x1 + x2) / 2, (y1 + y2) / 2, pros.extrudeHeight + pros.spaceHeight), new Vector3(x2, y2, pros.extrudeHeight)]; //画弧线
    let curve = new QuadraticBezierCurve3(...v3s);
    ...
    Object.assign(this.userData, pros); //线条光晕效果
    if (pros.hasHalo) {
  ...
  initHalo(geometry) {
    ...
    let color = new Color(colorToHex(this.userData.color));
    ...
    let psBufferGeometry = new BufferGeometry();
    ...
    let halo = new Points(psBufferGeometry, shaderMaterial);
    halo.dynamic = true;
    this.add(halo);
    this.halo = halo;
...
class Bar extends Object3D {
   ...
   */
  constructor(opt) {
    super(); //opt.additiveBlending=true
    let texture = opt.texture || new TextureLoader().load(opt.url);
    this.material = new MeshStandardMaterial({
      color: opt.color,
      emissive: opt.emissive || 0x000000,
      transparent: true,
      opacity: opt.opacity,
      blending: opt.additiveBlending ? AdditiveBlending : NormalBlending,
      side: DoubleSide,
      map: texture || null,
    ...
    this.geometry = opt.barType === 'box' ? new BoxGeometry(opt.size, opt.size, opt.value) : new CylinderGeometry(opt.size, opt.size, opt.value, 32);
  ...
  onmouseout(dispatcher, event) {
    ...
    new TWEEN.Tween(this.material.emissive).to(new Color(colorToHex(this.userData.emissive)), this.userData.hoverAnimaTime).start();
    dispatcher.dispatchEvent({
      type: 'mouseout',
      target: this,
      orgEvent: event
  ...
  onmouseover(dispatcher, event) {
    console.log(this.userData); // let size=1.5
    // new TWEEN.Tween(this.scale).to({x:size,y:size}, this.userData.hoverAnimaTime).start();
    //区域移入颜色
    new TWEEN.Tween(this.material.emissive).to(new Color(colorToHex(this.userData.hoverColor)), this.userData.hoverAnimaTime).start();
    dispatcher.dispatchEvent({
      type: 'mouseover',
      target: this,
      orgEvent: event
...
class Map3D {
  ...
  inintDataRange(dataRangeOpt) {
    ...
    this.dataRange.data.forEach(userData => {
      let opt = Object.assign({}, dataRangeOptClone, userData); //数据范围创建
      let range = new DataRange(opt); //区域与范围关联
      range.rangeAreas = [];
      this.dataRangeGroup.add(range); //根据范围调整区域颜色显示
    ...
    if (this.dataRange.data.length > 0) {
      if (this.dataRange.text[0]) {
        let txt = new Font3D$1(this.dataRange.text[0], {
      ...
      if (this.dataRange.text[1]) {
        let txt = new Font3D$1(this.dataRange.text[1], {
          color: this.dataRange.textColor
        });
        txt.position.add({
          x: 0,
  ...
  initBar(barOpt) {
    ...
    let barClone = Object.assign({
      extrudeHeight: this.extrude.amount
    }, this.bar);
    delete barClone.data;
    Bar.count = 0; //重新生成所有柱状图
    if (this.barGroup) {
  ...
  addCameraPosition(v3, time, delay, callback) {
    let v = new Vector3(v3.x, v3.y, v3.z);
    if (typeof time === 'number') {
  ...
  initDebug() {
    ...
    this.initStats();
    let helper = new DirectionalLightHelper(this.dirLight, 5);
    this.scene.add(helper);
    let spotLightHelper = new SpotLightHelper(this.spotLight);
    this.scene.add(spotLightHelper);
    let size = 300;
    let divisions = 40;
    let gridHelper = new GridHelper(size, divisions);
    this.scene.add(gridHelper);
    let axisHelper = new AxisHelper(50);
    ...
    if (intersects.length > 0) {
      ...
      if (this.selectedObj) {
        ...
        if (preSelectedObjID != this.selectedObj.id) {
          //老对象触发mouseout
          if (preSelectedObjID) {
            let preSelectedObj = this.scene.getObjectById(preSelectedObjID); //移出区域还原
  ...
  createArea(item) {
    ...
    let pros = Object.assign({
      ...
      //线颜色
      lineOpacity: this.area.lineOpacity,
      //线透明度
      ...
      //加载效果
      hasHoverHeight: this.area.hasHoverHeight,
      //有标注，选中区域不升高
      showText: this.area.showText,
      //是否显示地区名称
      textStyle: this.area.textStyle,
      //文字样式
...
Object.assign(EventDispatcher.prototype, {
  dispatchEvent: function (event) {
    ...
    if (listenerArray !== undefined) {
      ...
      let target = event.target; //通过原始事件构造自定义事件
      for (let a in event.orgEvent) event[a] = event.orgEvent[a]; //覆盖原始事件目标对象
